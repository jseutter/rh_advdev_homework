...
# Let system accounts in *-tasks-prod access images in *-tasks-dev
oc adm policy add-role-to-user system:image-puller system:serviceaccounts:${GUID}-tasks-prod -n ${GUID}-tasks-dev
...
oc set probe dc/tasks-blue --readiness --get-url=http://:8080/ --initial-delay-seconds=120 --timeout-seconds=5 -n ${GUID}-tasks-prod
oc set probe dc/tasks-blue --liveness --get-url=http://:8080/ --initial-delay-seconds=120 --timeout-seconds=5 -n ${GUID}-tasks-prod
...
oc set probe dc/tasks-green --readiness --get-url=http://:8080/ --initial-delay-seconds=120 --timeout-seconds=5 -n ${GUID}-tasks-prod
oc set probe dc/tasks-green --liveness --get-url=http://:8080/ --initial-delay-seconds=120 --timeout-seconds=5 -n ${GUID}-tasks-prod
# Setting 'wrong' VERSION. This will need to be updated in the pipeline
